...
volatile cfloat  AA[_Score][_Sedge] ALIGN(8) SECTION(".data_bank1");       // local operand array (ping)
volatile cfloat  BB[_Score][_Sedge] ALIGN(8) SECTION(".data_bank2");       // local operand array (pong)
volatile cfloat  Wn[_Sedge/2]       ALIGN(8) SECTION(".data_bank3");       // local Wn twiddles array //CHECK IF NEEDED!!
...
//KEEP FOLLOWING FOR NOW - BUT REMOVE IF NO NEED FOR Wn creation
...
//TAKE the below OUT IF NO NEED FOR Wn generation - which is likely
