...
int main(int argc, char *argv[])
{
	...
	do {
		if (me.corenum == 0)
		{
			// Wait for calc() call from the host. When a signal
		        // is detected in the mailbox, the loop
			...
			// Wait for "go" from the previous core. When a signal
			// is detected in the core's mailbox, the loop
			// is terminated and a call to the actual
			// calc() function is initiated.
			while (me.go_sync == 0) {};
		}
