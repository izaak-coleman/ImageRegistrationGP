...
process=./scalar_matrix_multiply_cpu   # the process to be timed
N=50               # number of runs the time is averaged over

# Executing process "./sorting" in a loop, to calculate average execution time
for ((i=1; i<=N; i++))
do
   #echo "Loop iteration $i"
   alg_time=$(time ($process >>output.txt 2>&1) 2>&1) #time the process, with time
