...
// Store the final transformation estimate in a matrix
...
double gpu_findTransformECC(InputArray templateImage,
  ...
  //accept only 1-channel (grayscale) images
  ...
  CV_Assert (criteria.type & TermCriteria::COUNT || 
  ...
  int paramTemp = 6;  // Parameters req. for affine transform
  ...
  for (int i = 1; (i <= numberOfIterations) && (fabs(rho-last_rho)>= termination_eps); i++)
  {
      ...
      // warp-back portion of the inputImage and gradients 
			// to the coordinate space of the templateImage
		...
		// download modified Mats to cpu versions
		// to give correct inputs for cpu bound functions
    ...
    Scalar imgMean, imgStd, tmpMean, tmpStd;
		...
		cuda::subtract(gpu_imageWarped, imgMean, gpu_imageWarped, 
		               gpu_imageMask);//zero-mean input
    ...
    templateZM = Mat::zeros(templateZM.rows, templateZM.cols, templateZM.type());
		...
		cuda::subtract(gpu_templateFloat, tmpMean, gpu_templateZM,  
		gpu_imageMask);//zero-mean template
