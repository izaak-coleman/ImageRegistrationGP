...
void image_jacobian_affine_ECC(const cuda::GpuMat& gpu_src1, const cuda::GpuMat& gpu_src2,
                                      const cuda::GpuMat& gpu_src3, const cuda::GpuMat& gpu_src4,
                                      Mat& dst);
...
double transferTime = 0; // GLOBAL
double warpAffineTime = 0; // GLOBAL
double meanStdDevTime = 0; // GLOBAL
double initTime = 0; // GLOBAL
double jacTime = 0; // GLOBAL
double normTime = 0; // GLOBAL

std::clock_t jacStart;
...
int main( int argc, char** argv )
{
  ...
  double cc = 0; // correlation coefficient
	...
	double totalTime = 0, withoutTransfer = 0;
  ...
  // Run find_transformECC to find the warp matrix
	...
	std::cout << std::endl << "Time of norm function: " << normTime << std::endl; 
...
void project_onto_jacobian_ECC(const Mat& src1, const Mat& src2, Mat& dst)
{
    ...
    */
    ...
    float* dstPtr = dst.ptr<float>(0);
        ...
        for (int i=0; i<dst.rows; i++){
            ...
            gpu_mat.upload(mat);
						...
						std::clock_t normclock;
						normclock = std::clock();
						norm = cuda::norm(gpu_mat, NORM_L2);
						normTime += (std::clock() - normclock)/(double)CLOCKS_PER_SEC;
	...
	jacTime += (std::clock() - jacStart)/(double)CLOCKS_PER_SEC;
}
...
double modified_findTransformECC(InputArray templateImage,
				 ...
				 Mat inputMask)
{
		std::clock_t function, initClock;
		function = std::clock();			// CUT TRANFER TIME
		initClock = std::clock();
    ...
    cuda::GpuMat gpu_preMask, gpu_imageMask;
		initTime += (std::clock() - initClock)/(double)CLOCKS_PER_SEC;
    ...
    for (int i = 1; (i <= numberOfIterations) && (fabs(rho-last_rho)>= termination_eps); i++)
    {
        ...
        // warp-back portion of the inputImage and gradients to the coordinate space of the templateImage
			...
			warpAffineTime += (std::clock() - function)/(double)CLOCKS_PER_SEC;
      ...
      Scalar imgMean, imgStd, tmpMean, tmpStd;
			...
			meanStdDevTime += (std::clock() - function)/(double)CLOCKS_PER_SEC;
      ...
      templateZM = Mat::zeros(templateZM.rows, templateZM.cols, templateZM.type());
			...
			image_jacobian_affine_ECC(gpu_gradientXWarped, gpu_gradientYWarped, gpu_Xgrid,
			gpu_Ygrid, jacobian);
